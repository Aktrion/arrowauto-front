...
import { ItemsApiService } from '@app/features/settings/services/items-api.service';
import { DataGridComponent } from '@shared/components/data-grid/data-grid.component';
import { ColumnDef } from '@shared/components/data-grid/data-grid.interface';
import { ICONS } from '@shared/icons';

@Component({
  selector: 'app-items-configuration',
  standalone: true,
  imports: [FormsModule, LucideAngularModule, TranslateModule, DataGridComponent],
  templateUrl: './items-configuration.html',
})
export class ItemsConfiguration extends BaseListDirective<any, any, any> {
  icons = ICONS;
  ...
  protected override onDelete(item: any): void {
    ...
    this.itemsApiService.deleteOne(id).subscribe({
      ...
      error: () =>
        this.notificationService.error(this.translate.instant('ITEMS.TOAST.DELETE_FAILED')),
    });
  }
  ...
  createItem(): void {
    if (!this.newItem.partNumber || !this.newItem.partDescription || this.newItem.price == null)
      return;
    ...
    this.itemsApiService
      ...
      .subscribe({
        ...
        error: () =>
          this.notificationService.error(this.translate.instant('ITEMS.TOAST.CREATE_FAILED')),
      });
  }
  ...
  saveEditItem(): void {
    ...
    this.itemsApiService
      ...
      .subscribe({
        ...
        error: () =>
          this.notificationService.error(this.translate.instant('ITEMS.TOAST.UPDATE_FAILED')),
      });
  }
